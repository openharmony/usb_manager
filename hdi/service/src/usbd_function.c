...
static int SendCmdToService(const char *name, int cmd, unsigned char funcMask)
{
    ...
    if (data == NULL || reply == NULL) {
        printf("data or reply err\n");
        status = HDF_FAILURE;
        HdfRemoteServiceRecycle(service);
        return status;
    }
    if (!HdfSbufWriteInt8(data, funcMask)) {
        printf("UsbEcmRead HdfSbufWriteInt8 error");
        status = HDF_FAILURE;
        HdfSBufRecycle(data);
    ...
    if (status) {
        printf("serice %s dispatch cmd : %d error\n", name, cmd);
    }
    HdfSBufRecycle(data);
    HdfSBufRecycle(reply);
    HdfRemoteServiceRecycle(service);
