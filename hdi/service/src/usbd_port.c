...
static int SetDefaultPort(struct UsbdService *service)
{
    return SetPort(DEFAULT_PORT_ID, DEFAULT_POWER_ROLE, DEFAULT_DATA_ROLE, service);
}

int QueryPort(int *portId, int *powerRole, int *dataRole, int *mode, struct UsbdService *service)
{
    int ret = SetDefaultPort(service);
    if (ret) {
        HDF_LOGE("%{public}s: SetDefaultPort error", __func__);
        return HDF_FAILURE;
    }
